...
impl Session {
  ...
  pub fn send_message<M: Into<Message>>(
    ...
    unsafe {
      ...
      if self.producer_pointer == 0 {
        let status =
          tibco_ems_sys::tibemsSession_CreateProducer(self.pointer, &mut local_producer, dest);
        match status {
          tibems_status::TIBEMS_OK => {
      ...
      let status =
        tibco_ems_sys::tibemsMsgProducer_SendToDestination(self.producer_pointer, dest, msg);
      match status {
  ...
  pub fn request_reply<M: Into<Message>>(
    ...
    unsafe {
      match &destination {
        Destination::Queue(name) => {
          let status =
            tibco_ems_sys::tibemsSession_CreateTemporaryQueue(self.pointer, &mut reply_dest);
          match status {
            tibems_status::TIBEMS_OK => {
        ...
        Destination::Topic(name) => {
          let status =
            tibco_ems_sys::tibemsSession_CreateTemporaryTopic(self.pointer, &mut reply_dest);
          match status {
            tibems_status::TIBEMS_OK => {
  ...
  unsafe {
    match message {
      ...
      Message::ObjectMessage(msg) => {
        ...
        let body_ptr = content.as_ptr() as *const c_void;
        let status =
          tibco_ems_sys::tibemsObjectMsg_SetObjectBytes(msg_pointer, body_ptr, body_size as u32);
        match status {
          tibems_status::TIBEMS_OK => trace!("tibemsObjectMsg_SetObjectBytes: {:?}", status),
        ...
        for (key, val) in msg.body.clone() {
          ...
          match val {
            ...
            TypedValue::Float(value) => {
              let status =
                tibco_ems_sys::tibemsMapMsg_SetFloat(msg_pointer, c_name.as_ptr(), value);
              match status {
                tibems_status::TIBEMS_OK => {
            ...
            TypedValue::Double(value) => {
              let status =
                tibco_ems_sys::tibemsMapMsg_SetDouble(msg_pointer, c_name.as_ptr(), value);
              match status {
                tibems_status::TIBEMS_OK => {
    ...
    if let Some(headers) = header {
      ...
      for (key, val) in &headers {
        ...
        match val {
          ...
          TypedValue::Integer(value) => {
            let status =
              tibco_ems_sys::tibemsMsg_SetIntProperty(msg_pointer, c_name.as_ptr(), *value);
            match status {
              tibems_status::TIBEMS_OK => {
          ...
          TypedValue::Long(value) => {
            let status =
              tibco_ems_sys::tibemsMsg_SetLongProperty(msg_pointer, c_name.as_ptr(), *value);
            match status {
              tibems_status::TIBEMS_OK => {
  ...
  unsafe {
    ...
    match msg_type {
        ...
        loop {
          ...
          match status {
            tibems_status::TIBEMS_OK => {
              ...
              let status =
                tibco_ems_sys::tibemsMapMsg_GetString(msg_pointer, buf_ref, &mut val_buf_ref);
              match status {
                tibems_status::TIBEMS_OK => {
                ...
                tibems_status::TIBEMS_CONVERSION_FAILED => {
                  ...
                  let status =
                    tibco_ems_sys::tibemsMapMsg_GetMapMsg(msg_pointer, buf_ref, &mut msg2);
                  match status {
                    tibems_status::TIBEMS_CONVERSION_FAILED => {
                    ...
                    tibems_status::TIBEMS_OK => {
                      ...
                      match &mut raw_message {
                        ...
                        Message::MapMessage(msg) => {
                          msg.pointer = None;
                          body_entries
                            .insert(header_name.to_string(), TypedValue::Map(msg.clone()));
                        }
                      }
                    }
      ...
      tibemsMsgType::TIBEMS_OBJECT_MESSAGE => {
        ...
        let mut result_size: u32 = 0;
        let status =
          tibco_ems_sys::tibemsObjectMsg_GetObjectBytes(msg_pointer, &buf_ref, &mut result_size);
        match status {
          tibems_status::TIBEMS_OK => trace!("tibemsObjectMsg_GetObjectBytes: {:?}", status),
    ...
    loop {
      ...
      match status {
        tibems_status::TIBEMS_OK => {
          ...
          let status =
            tibco_ems_sys::tibemsMsg_GetStringProperty(msg_pointer, buf_ref, &val_buf_ref);
          match status {
            tibems_status::TIBEMS_OK => {
